...
def get_tricks(  
    ...
    if all(len(ch) == 1 for ch in input_parts):  
        ...
        if count > 4 and type in sentence_templates:  
            count_str = str(count)  
            if count_str in sentence_templates[type]:  
                template = sentence_templates[type][count_str]  
                try:  
                    trick = template.format(*names[:int(count_str)])  
                except Exception as e:  
                    print(f"Template formatting error: {e}")  
                    trick = "Error while formatting template."  
                return {"trick": trick}  
            else:  
                print(f"No template found for count {count} in {type}")  
                return {"trick": f"Template missing for count {count}."}  
