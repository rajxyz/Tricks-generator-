...
def load_templates(): """Load templates from JSON file.""" templates_path = os.path.join(os.path.dirname(os.path.abspath(file)), "..", "templates.json")
...
def load_actors(letter=None): """Load Bollywood actors from JSON file. Filter by letter if provided.""" file_path = os.path.join(os.path.dirname(os.path.abspath(file)), "..", "bollywood-actor.json")
...
def get_next_actors(letters): """Fetch the next available actor for each entered letter in a cyclic manner.""" selected_actors = [] for letter in letters: actors = load_actors(letter) if actors: index = actor_index[letter] % len(actors)  # Cycle through actors selected_actors.append(actors[index]) actor_index[letter] += 1  # Move to next actor for next call return selected_actors

def generate_trick_sentence(actors, templates): """Generate a trick sentence using actor names and a template.""" if not actors: return "No actors found for the entered letters."
...
@router.get("/api/tricks") def get_tricks( type: str = Query(None, description="Type of trick (e.g., actors, cricketers)"), letter: str = Query(None, description="Comma-separated letters") ): """API Endpoint to generate tricks based on user input.""" print(f"Request received: type={type}, letter={letter}")
