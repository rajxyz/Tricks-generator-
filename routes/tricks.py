import json import os import random from fastapi import APIRouter, Query

router = APIRouter()

Load templates for dynamic sentence generation (stored in a separate JSON file)

def load_templates(): templates_path = os.path.join(os.path.dirname(os.path.abspath(file)), "..", "templates.json") with open(templates_path, "r", encoding="utf-8") as f: return json.load(f)
...
def generate_trick_sentence(actors, templates): if not actors: return "No actors found for this letter."

num_actors = len(actors)

# Select template category based on number of actors
...
@router.get("/api/tricks") def get_tricks( type: str = Query(None, description="Type of trick (e.g., actors, cricketers)"), letter: str = Query(None, description="Starting letter") ): templates = load_templates()  # Load templates from JSON file
