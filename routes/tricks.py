...
def get_next_entities(trick_type, letters):
    ...
    for letter in letters:
        ...
        if entities:
            ...
            logger.debug(f"Selected entity {entities[idx]} for letter '{letter}'")
        else:
...
def generate_trick_sentence(entities, templates):
    try:
        ...
        logger.debug(f"Generating trick sentence with last name: {last_name}")
...
def generate_trick_with_topic(topic, entities, templates):
    try:
        ...
        logger.debug(f"Generating trick with topic '{topic}' and last entity: {last_entity}")
...
def get_tricks(
    ...
    try:
        ...
        # Actors, Cricketers, Animals, Professions: Similar pattern
        if type in [TrickType.actors, TrickType.cricketers, TrickType.animals, TrickType.professions]:
            ...
            logger.debug(f"Loaded templates from {template_path}")
        ...
        # Abbreviations special handling
